...
5. **Probar endpoints del microservicio con PowerShell:**
   ```powershell
   # Obtener todos los mangas
   Invoke-RestMethod -Uri "http://localhost:8080/api/mangas"
   
   # Obtener un manga específico
   Invoke-RestMethod -Uri "http://localhost:8080/api/mangas/1"
   
   # Buscar por autor (parámetro: nombre)
   Invoke-RestMethod -Uri "http://localhost:8080/api/mangas/search/autor?nombre=Tatsuki"
   
   # Buscar por género (parámetro: tipo)
   Invoke-RestMethod -Uri "http://localhost:8080/api/mangas/search/genero?tipo=Shonen"
   ...
   Invoke-RestMethod -Uri "http://localhost:8080/api/mangas" -Method Post -Body $nuevoManga -ContentType "application/json"
   ```

### Paso 2: Probar la API Externa (JSONPlaceholder)

Antes de ejecutar la aplicación Android, puedes verificar que la API externa esté disponible:

```powershell
# Obtener todos los posts
Invoke-RestMethod -Uri "https://jsonplaceholder.typicode.com/posts" | Select-Object -First 5
...
cd C:\Users\irina\Desktop\APP\AppNubeComics_003V_Grupo15-main
...
cd C:\Users\irina\Desktop\APP\AppNubeComics-Backend
...
![Tests Android 1](UnitTest.png)
*Tests de repositorios (PostRepository y MangaRepository)*

![Tests Android 2](UnitTest2.png)
*Tests de ViewModels con output detallado*

![Tests Android 3](UnitTest3.png)
*Resumen de ejecución: 22 tests pasando*

**Test de API Externa:**

![Test API Externa](TestApiExterna.png)
*Integración con JSONPlaceholder funcionando correctamente*
...
✅ **APK Firmado:** `NubeComics-v1.0-release.apk` (23.87 MB)
- Ubicación: Raíz del proyecto
- Versión: 1.0
- Firmado digitalmente con keystore propio

✅ **Keystore:** `nubecomics-release-key.jks`
- Ubicación: Raíz del proyecto
- Alias: `nubecomics`
...
signingConfigs {
    create("release") {
        storeFile = file("../nubecomics-release-key.jks")
        storePassword = "nubecomics2024"
        keyAlias = "nubecomics"
        keyPassword = "nubecomics2024"
    }
}
